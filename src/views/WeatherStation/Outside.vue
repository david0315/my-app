<template>
  <el-card shadow="hover">
    ...
    <el-table :data="info" :header-cell-style="{background:'#f5f7fa'}" border>
      <el-table-column label="参数" align="center">
        <template slot-scope="scope">
          <!-- 风向不需要折线图 -->
          <el-button
            type="text"
            @click="nameClick(scope)"
            v-if="scope.row.key != 'winddirection'"
...
export default {
  ...
  methods: {
    ...
    getChartData(id, key, date) {
      ...
      this.axios({
        url:
          "http://60.190.23.22:8889/fertilizer_distributor/api/do.jhtml?router=appApiService.getData",
        params: {
          fd_id: id,
          date: date
        }
